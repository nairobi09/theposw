...
namespace thepos
{
    internal class paymentKovan
    {
        ...
        public int requestKovanCashAuth(int tAmount, int tFreeAmount, int tTaxAmount, int tTax, int tServiceAmt, String receipt_type, out PaymentCash pCash)
        {
            ...
            if (receipt_type == "1") tReceiptType = "00";
            else if (receipt_type == "2") tReceiptType = "01";
            else if (receipt_type == "S") tReceiptType = "00";
            ...
            tcode = Encoding.Default.GetBytes("41");


            tid = Encoding.Default.GetBytes(mVanTID);              //tid
            halbu = Encoding.Default.GetBytes(tReceiptType);          // 00소득공제 01지출증빙
            tamt = Encoding.Default.GetBytes(tAmount.ToString("000000000"));   //거래금액:실제 결제되는 금액
            ori_date = Encoding.Default.GetBytes("      ");      //취소시 원거래일자
            ori_authno = Encoding.Default.GetBytes("            ");//취소시 원거래 승인번호 12자리 우측 스페이스 채움
            tran_serial = Encoding.Default.GetBytes(get_today_time() + "      ");//거래일련번호 망취소시 거래일련번호로 사용됨므로 중복되면 망취소 안됨(하루에 중복되는 거래일련번호로 사용시 망취소 에러). 
            idno = Encoding.Default.GetBytes(new String(' ', 33)); //33 스페이스, 현금영수증거래/ 수표조회시 식별번호
            ...
            if (ResCd == "0000")
            {
                mPaymentCash.tran_serial = Encoding.Default.GetString(rTranSerial).Substring(0, 12).Trim(); // 거래번호
                mPaymentCash.tran_date = Encoding.Default.GetString(rTranDate).Substring(0, 6);     // 거래일시
                mPaymentCash.auth_no = Encoding.Default.GetString(rAuthNo).Substring(0, 12).Trim();         // 승인번호
                mPaymentCash.issued_method_no = Encoding.Default.GetString(rCardno).Substring(0, 18).Trim();         // 마스킹 카드번호
        ...
        public int requestKovanCashCancel(PaymentCash pCashAuth, out PaymentCash pCashCancel)
        {
            ...
            tcode = Encoding.Default.GetBytes("42");


            string tReceiptType = "";

            if (pCashAuth.receipt_type == "1") tReceiptType = "10";
            else if (pCashAuth.receipt_type == "2") tReceiptType = "11";
            else if (pCashAuth.receipt_type == "S") tReceiptType = "10";


            tid = Encoding.Default.GetBytes(mVanTID);              //tid
            halbu = Encoding.Default.GetBytes(tReceiptType);          // 00소득공제 01지출증빙
            tamt = Encoding.Default.GetBytes(pCashAuth.amount.ToString("000000000"));   //거래금액:실제 결제되는 금액
            ori_date = Encoding.Default.GetBytes(pCashAuth.tran_date);      //취소시 원거래일자
            ori_authno = Encoding.Default.GetBytes(pCashAuth.auth_no + new String(' ', 33 - pCashAuth.auth_no.Length));//취소시 원거래 승인번호 12자리 우측 스페이스 채움
            tran_serial = Encoding.Default.GetBytes(pCashAuth.tran_serial + "      ");//거래일련번호 망취소시 거래일련번호로 사용됨므로 중복되면 망취소 안됨(하루에 중복되는 거래일련번호로 사용시 망취소 에러). 
            //idno = Encoding.Default.GetBytes(pCashAuth.issued_method_no + new String(' ', 33 - pCashAuth.issued_method_no.Length)); //33 스페이스, 현금영수증거래/ 수표조회시 식별번호
            idno = Encoding.Default.GetBytes(new String(' ', 33)); //33 스페이스, 현금영수증거래/ 수표조회시 식별번호

            amt_flag = Encoding.Default.GetBytes("   ");//미사용 스페이스

            //세금 봉사료 비과세 계산해야함. 계산안하면 0원 
            tax_amt = Encoding.Default.GetBytes("000000000");  //세금 
            sfee_amt = Encoding.Default.GetBytes("000000000"); //봉사료 
            free_amt = Encoding.Default.GetBytes("000000000"); //비과세 
            ...
            if (ResCd == "0000")
            {
                pCashCancel.tran_serial = Encoding.Default.GetString(rTranSerial).Substring(0, 12).Trim(); // 거래번호
                pCashCancel.tran_date = Encoding.Default.GetString(rTranDate).Substring(0, 6);     // 거래일시
                pCashCancel.issued_method_no = Encoding.Default.GetString(rCardno).Substring(0, 18).Trim();         // 마스킹 카드번호
